...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/17 19:29:18 by mboulara          #+#    #+#             */
/*   Updated: 2020/01/06 14:01:12 by mboulara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char			*apply_hex_width(t_format info, char *str, char c)
{
	int			len;
...
size_t			x_handler(t_format info, va_list *args)
{
	...
	s = info.width > 0 ? apply_hex_width(info, s, c) : s;
	len = ft_strlen(s);
	write(info.fd, s, len);
	return (len);
}
